...
export default {
    ...
    parse(value) {
        ...
        if (value instanceof Date) {
            ...
            case 'string':
                date = new Date()
                // 防止设置的月份最大天数小于当前月份的最大天数，比如今天是3月31号，解析的4月1号，但是4月没有31号，导致月份被设置为5月
                date.setDate(1)
    ...
    apart(future, format, current) {
        ...
        if (future instanceof Date && current instanceof Date) {
            ...
            let Y2 = future.getFullYear()
            let M2 = future.getMonth()
            let D2 = future.getDate()
            let Y = Math.max(Y2 - Y1 + (M1 * 100 + D1 > M2 * 100 + D2 ? -1 : 0), 0)
            let M = Math.max((Y2 - Y1) * 12 + M2 - M1 + (D1 > D2 ? -1 : 0), 0)
            let S = Math.max(future.getTime() - current.getTime(), 0)
            let s = Math.max(S / 1e3, 0)
            let m = Math.max(s / 60, 0)
            let h = Math.max(m / 60, 0)
            let D = Math.max(h / 24, 0)
            ...
            option.forEach((item, index) => {
                if (format.indexOf(item.key) > -1) {
                    ...
                    if (item.size && option[index - 1].value > -1) {
                        value = value % item.size
                    }
                    item.value = Math.floor(value)
                    let optional = new RegExp(`\{([${item.key}])+[^\}]+\}`).test(format)
                    let optionalText = RegExp['$&']
